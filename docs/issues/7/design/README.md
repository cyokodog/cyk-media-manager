...
> **未採用**: `FolderPicker.dc.html` のディレクトリブラウザ（参照ダイアログ）は現時点では実装しない。本ツールはスタンドアローンで利用する前提のため、フォルダ指定はテキスト入力と履歴のみとした。アートボードは将来の参照用に残している。
...
- 履歴の保持件数。4件で描画している
- ~~ディレクトリブラウザの画像枚数表示~~ → ディレクトリブラウザ自体を未採用としたため不要
